...
rt <- raster("/Volumes/Jellyfish/Dropbox/Manuscripts/Bio-ORACLE Across Climate Changes/Bioclimatic Layers/Present/Surface_HR/LongTerm/Ocean.temperature.Surface.Var.Lt.Max.tif")
rt <- raster::resample(rt,region.as.raster, method="bilinear")

rt <- rt / max(getValues(rt),na.rm=TRUE)

levelplot(rt,margin=FALSE,xlab=NULL, ylab=NULL, at=seq(0,1,length.out=100), par.settings = rasterTheme(region = matlab.like(n = 100)))
